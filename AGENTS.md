...
<!--
  ...
  The previous AGENTS.md was ~51% destroyed by a Windows generation tool that
  ...
  conventions — `.Codex/rules/**`, `package.json`, `.npmrc`, the live
  ...
  `.Codex/rules/**` and the ADRs in `docs/adr/` are the source of truth; this
...
# AGENTS.md — Disaster Recovery

Behavioural anchor for this repo. It does NOT duplicate the detailed rules —
it points you to the right file. The deep rules live in `.Codex/rules/**`,
the decisions live in `docs/adr/`, and the running log lives in `MEMORY.md`.

- `.Codex/rules/**` — the hard rules (compliance, Australian English,
...
| Topic                                | File                                              |
| ------------------------------------ | ------------------------------------------------- |
| Compliance / legal (banned phrases)  | @.Codex/rules/compliance.md                      |
| Business model / who-bills-who       | @.Codex/rules/business-rules.md                  |
| Privacy / data classification        | @.Codex/rules/privacy.md                         |
| Australian English (spelling)        | @.Codex/rules/australian-english.md              |
| Dev environment / build quirks       | @.Codex/rules/dev-environment.md                 |
...
If a rule used to live in AGENTS.md and isn't here now, it moved to one of those files — nothing was deleted.
...
- **Model:** Network-orchestrator — DR does NOT do restoration work; IICRC-certified
  ...
  live in @.Codex/rules/business-rules.md.
...
/.Codex/rules              compliance, au-english, privacy, biz, dev-env
/.Codex/skills             Matt Pocock skills (DR-724)
```
...
- Don't claim something is done until it's verified (lint + typecheck + tests
  ...
  @.Codex/rules/dev-environment.md §7). When in doubt, stop and ask for
...
- The first caller utterance after pickup MUST be the APP 8 consent prompt
  (canonical wording in @.Codex/rules/compliance.md §3).
...
- Rollback = flip the env var + redeploy the same commit. See
  @.Codex/rules/dev-environment.md §5 and @docs/adr/ADR-004-feature-flag-strategy.md.
...
- Every state-mutating API route appends to `compliance_events` (structured,
  ...
  @.Codex/rules/compliance.md §9 and @docs/adr/ADR-013.
...
- Do NOT write US English ANYWHERE. See `.Codex/rules/australian-english.md`.
- Do NOT reintroduce a banned compliance phrase — see @.Codex/rules/compliance.md §1.
...
1. **New feature?** Start with @.Codex/rules/business-rules.md (is it legal /
...
this file conflicts with a `.Codex/rules/**` file or an ADR, that file wins.
...
- PR title + commit format: `<type>(<scope>): <subject>` (commitlint-enforced).
  ...
  `security`, `style`. See @.Codex/rules/dev-environment.md §8.
