/**
 * Disaster Recovery — AI Front Desk configuration (SKELETON / EXAMPLE).
 *
 * Copy to `frontdesk.config.ts` and fill real values when the shared
 * `@nexus/front-desk` package lands. Ships dark until `DR_FRONT_DESK_ENABLED=true`.
...
export interface FrontDeskConfig {
  /** Stable project key. */
  project: string;
  /** Enable flag — reads process.env.DR_FRONT_DESK_ENABLED. Default off. */
  enabled: boolean;
...
export const config: FrontDeskConfig = {
  ...
  enabled: process.env.DR_FRONT_DESK_ENABLED === 'true',
  brand: { assistantName: 'the Disaster Recovery intake line', accent: '#000000' /* TODO brand accent */ },
  voice: { provider: 'elevenlabs', voiceId: 'TODO_DISASTER_RECOVERY_VOICE_ID', locale: 'en-AU' },
  knowledge: { kind: 'prompt', source: 'TODO — this brand\'s knowledge base' },
  channels: ['phone-in', 'chat', 'phone-out', 'voice'],
  vendor: 'elevenlabs',
  phoneNumber: null,
  tools: ['job intake', 'dispatch', 'sms', 'calendar/scheduling', 'human handoff'],
  compliance: {
    businessIdentity: 'Disaster Recovery',
