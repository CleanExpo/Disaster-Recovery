# Monthly Citation Monitoring Checklist

**Purpose:** After the initial citation cleanup, old NAP data (Name/Address/Phone) can persist in platform caches and aggregator databases for 4–12 weeks. This checklist ensures straggler listings are caught and corrected on a monthly cadence.

**Frequency:** Once per month — suggested on the first Monday of each month.
...
## 2. What to Check on Each Platform
...
## 3. Monthly Check Procedure

Follow these steps each month:

1. **Open this checklist** and note today's date in the tracking log.

2. **Start with Google Business Profile.**
   - Log in to Google Business Profile Manager.
   - Check each data point in Section 2.
   - If any field is incorrect, edit it directly in GBP.
   - Screenshot any issues found before correcting.

3. **Check Apple Maps.**
   - Search for the business in Apple Maps (browser or device).
   - If data is incorrect, go to Maps Connect (mapsconnect.apple.com) and submit a correction.
   - Note: Apple Maps corrections can take 3–5 business days.

4. **Check Bing Places.**
   - Log in to Bing Places for Business.
   - Verify all fields in Section 2.
   - Edit directly if incorrect.

5. **Check Australian Directory Platforms (True Local, Localsearch, Yellow Pages, White Pages, Hotfrog).**
   - Search for the business on each platform.
   - If a listing exists with incorrect data, log in and update, or use the "Suggest an Edit" / "Claim Listing" flow if not already owned.
   - If the listing cannot be edited (no claim option), note it in the tracking log under "Escalation Required".

6. **Check LinkedIn and Facebook.**
   - Review the "About" section of each.
   - Update phone, website, and description if needed.

7. **Check Data Aggregators (RocketReach, ZoomInfo, Wheree, Osogbo, Ensun).**
   - Search the business name on each aggregator.
   - If outdated data appears, submit a correction via their respective forms.
   - Log submission date — aggregator corrections typically take 4–8 weeks to propagate.

8. **Update the tracking log** (see Section 4) with results for every platform checked.

9. **Flag any unresolved items** for escalation (see Section 5).

---

## 4. Tracking Log Template
...
- **Pending** — Correction submitted, awaiting propagation
...
- The website URL is `https://disasterrecovery.com.au`
- At least one relevant disaster restoration category is present
- Hours reflect 24/7 availability

A listing remains **unresolved** if any single NAP field still shows old data, even if other fields are correct.

### Cache persistence note

Some platforms (particularly aggregators and data resellers) maintain internal caches that may serve old data to third parties even after the source listing is corrected. If a correction has been applied at the source but old data still appears in searches, wait a further 4–6 weeks before re-escalating — the correction may still be propagating through downstream caches.
