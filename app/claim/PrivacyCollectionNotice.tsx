...
export function PrivacyCollectionNotice() {
  ...
  return (
    <div
      ...
      lang={language}
    >
      {loading && <p className="text-xs text-blue-500 italic">Translating…</p>}
      <p>
        <strong>National Restoration Professionals Group Pty Ltd</strong> (ABN 85 151 794 142),
        trading as <strong>Disaster Recovery</strong>
        {isEnglish ? (
          <>
            , is collecting your personal information on this form to match your claim with a
            certified IICRC restoration contractor in our network.
          </>
        ) : (
          <>
            .{' '}
            {texts.p1.replace(
              /^National Restoration Professionals Group Pty Ltd \(ABN 85 151 794 142\), trading as Disaster Recovery[,.]?\s*/i,
              '',
            )}
          </>
        )}
      ...
      <p>
        {texts.p5} <strong>oaic.gov.au</strong> or on <strong>1300 363 992</strong>.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-3 py-1">
        ...
        />
        <Label
          htmlFor="privacyCollectionNotice"
