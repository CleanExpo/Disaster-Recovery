...
import { PaymentAuditLogger, PaymentValidator, PRICING_CONSTANTS } from '@/lib/payment-security';
...
// DR-780: Subscription amount now sourced from PRICING_CONSTANTS.SUBSCRIPTION_TIERS
// in src/lib/payment-security.ts (IMMUTABLE PRICING CONSTANTS, dated 2026-04-07).
// Canonical amounts (in cents): 25KM=$495, 50KM=$795, 75KM=$995, 100KM=$1,295.
// Frequency multipliers (MONTHLY=1, QUARTERLY=2.85, ANNUAL=10.5) are applied
// by PaymentValidator.calculateSubscriptionAmount.
//
// RURAL tier is not yet defined in PRICING_CONSTANTS — treated as TIER_100KM
// pricing pending the canonical rate decision. Tracked under DR-780.
function getSubscriptionAmount(tier: string, frequency: string): number {
  ...
  const canonicalFrequency = (
    ['MONTHLY', 'QUARTERLY', 'ANNUAL'].includes(frequency) ? frequency : 'MONTHLY'
  ) as 'MONTHLY' | 'QUARTERLY' | 'ANNUAL';
  const result = PaymentValidator.calculateSubscriptionAmount(canonicalTier, canonicalFrequency);
  // PaymentValidator returns cents; legacy callers of this function (the Prisma
  // contractorSubscription.amount field) historically stored dollars. Preserve
  // that contract here by dividing back to dollars to avoid silently 100xing
  // existing rows. Subscription charges that hit Stripe go through STRIPE_PRICES
  // (price IDs), not this number.
