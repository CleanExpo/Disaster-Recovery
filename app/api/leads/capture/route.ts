...
import { leadCaptureSchema } from '@/lib/api/validations';
import {
  apiSuccess,
  apiValidationError,
  apiBadRequest,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // --- Zod validation (was: zero validation, raw data.* into prisma) ---
    ...
    if (leadScore < 50) {
      return apiBadRequest('Lead does not meet quality requirements');
    }
    ...
    // Create lead in database — all fields now validated by Zod
    ...
    return apiInternalError(error, 'leads/capture');
  }
}
