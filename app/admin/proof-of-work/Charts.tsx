...
export type WorkTypeDatum = { name: string; value: number };

export function VerificationStatusChart({ data }: { data: StatusDatum[] }) {
  if (data.length === 0) {
    return (
...
export function WorkTypeChart({ data }: { data: WorkTypeDatum[] }) {
  if (data.length === 0) {
    return (
      <div className="flex h-full items-center justify-center text-sm text-gray-400">
        No claims yet
