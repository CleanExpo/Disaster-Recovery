...
const WeightedAllocationEngine: React.FC = () => {
  ...
  const [allocationWeights, setAllocationWeights] = useState<AllocationWeight[]>([
    ...
    { factor: 'Specialization', description: 'Service type expertise', weight: 0.10, impact: 'low', adjustable: true }
  ]);
  ...
  const runAllocation = () => {
    ...
    const factorsAnalyzed: FactorAnalysis[] = allocationWeights.map((weight, index) => {
      const factors = ['kpiScore', 'distance', 'capacity', 'leadSharePercentage', 'specialization'];
