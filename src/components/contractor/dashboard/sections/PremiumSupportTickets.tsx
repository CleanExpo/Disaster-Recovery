...
export function PremiumSupportTickets({ tickets = [] }: SupportTicketsProps) {
  const [selectedTab, setSelectedTab] = useState<'all' | 'open' | 'in_progress' | 'resolved'>(
    'all',
  );
      ...
      : [
          {
            ...
            description:
              'Getting error when trying to upload new insurance certificate. The system shows "File type not supported" even though it\'s a PDF.',
            status: 'OPEN',
            priority: 'HIGH',
            category: 'Compliance',
          ...
          },
          {
            ...
            description:
              'Made payment 3 days ago but balance not updated. Transaction ID: TXN-98765432',
            status: 'IN_PROGRESS',
            priority: 'MEDIUM',
            category: 'Billing',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'In Progress',
            value: animatedNumbers.inProgress,
            icon: Clock,
          ...
          {
            label: 'Resolved',
            value: animatedNumbers.resolved,
            icon: CheckCircle,
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur-xl border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
              {/* Gradient Background */}
              <div
                className={cn('absolute inset-0 opacity-5 bg-gradient-to-br', `${stat.gradient}`)}
              />
              ...
              <div className="relative p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  {stat.trend && (
                    ...
                    >
                      <TrendingUp
                        className={cn('w-3 h-3', !stat.trend.startsWith('+') && 'rotate-180')}
                      />
                      {stat.trend}
                    </div>
                  )}
          ...
          <div className="flex overflow-x-auto">
            {[
              ...
              <button
                key={tab.id}
                onClick={() =>
                  setSelectedTab(tab.id as 'all' | 'open' | 'in_progress' | 'resolved')
                }
                className={cn(
        ...
        {[
          {
            icon: MessageSquare,
            label: 'Live Chat',
            description: 'Start instant chat',
