...
export class ImageSEOAgent {
  ...
  static optimizeImage(
    ...
    return {
      ...
      schemaData,
      formats: {
        webp: image.formats.webp ?? '',
        avif: image.formats.avif,
        jpg: image.formats.jpg ?? '',
      },
      responsiveSizes: {
  ...
  private static extractKeywords(image: ImageAsset, context: any) {
    ...
    if (context.location) {
      longTailKeywords = longTailKeywords.map((keyword) => `${keyword} ${context.location}`);
    }
   ...
   */
  private static generateAltText(image: ImageAsset, keywords: any, context: any): string {
    const templates = {
    ...
    const template =
      templates[image.category as keyof typeof templates] ||
      image.alt ||
      'Professional disaster recovery service';
   ...
   */
  private static generateTitle(image: ImageAsset, keywords: any, context: any): string {
   ...
   */
  private static generateCaption(image: ImageAsset, keywords: any, context: any): string {
    const captions = {
    ...
    return (
      captions[image.category as keyof typeof captions] ||
      image.description ||
      'Professional disaster recovery and restoration services'
    );
  }
   ...
   */
  private static generateSchemaData(image: ImageAsset, keywords: any, context: any): any {
    return {
  ...
  private static determinePriority(image: ImageAsset): 'high' | 'medium' | 'low' {
    if (
      image.category &&
      image.category.includes('damage') &&
      image.tags &&
      image.tags.includes('emergency')
    ) {
      return 'high';
    }
   ...
   */
  private static determineOptimalPlacement(
    image: ImageAsset,
  ): 'above-fold' | 'in-content' | 'gallery' {
