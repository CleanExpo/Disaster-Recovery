...
export class AIService implements IAIService {
  ...
  private initializeMetrics(): void {
    this.metrics = {
      ...
      totalCost: 0,
      cacheHitRate: 0,
      providerMetrics: {} as AIMetrics['providerMetrics'],
    };
  ...
  private selectProvider(context: AITaskContext, preferredProvider?: AIProvider): AIProvider[] {
    ...
    providers.push(
      ...routing.preferredProviders.filter(
        (p) => this.isProviderAvailable(p) && !providers.includes(p),
      ),
    );

    // Add fallback providers
    providers.push(
      ...routing.fallbackProviders.filter(
        (p) => this.isProviderAvailable(p) && !providers.includes(p),
      ),
    );
  ...
  private updateCircuitBreaker(provider: AIProvider, error: any): void {
    ...
    if (breaker.failures >= 3) {
      breaker.isOpen = true;
      logger.warn('system', `Circuit breaker opened for provider: ${provider}`, {
      ...
      if (success) {
        ...
        if (response) {
          ...
          providerMetrics.averageResponseTime =
            (providerMetrics.averageResponseTime * (providerMetrics.successes - 1) +
  ...
  async generateResponse(
    ...
    if (this.config.cache.enabled) {
      ...
      if (cached) {
        logger.info('system', 'AI response served from cache', { taskType: context.type });
    ...
    throw (
      lastError ||
      new AIServiceError('All AI providers failed to respond', 'ALL_PROVIDERS_FAILED', false)
    );
  }
