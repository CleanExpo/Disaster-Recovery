...
export async function generateAllImages() {
  ...
  // Process by priority
  const results = {
    priority1: await processPriorityGroup(
      priority1,
      coordinator,
      pipeline,
      'Priority 1 - Hero Images',
    ),
    priority2: await processPriorityGroup(
      priority2,
      coordinator,
      pipeline,
      'Priority 2 - Marketing/Training',
    ),
    priority3: await processPriorityGroup(
      priority3,
      coordinator,
      pipeline,
      'Priority 3 - CRM/Documentation',
    ),
  };
...
async function processPriorityGroup(
  ...
  for (const requirement of requirements) {
    ...
    console.log(
      `      Dimensions: ${requirement.dimensions.width}x${requirement.dimensions.height}`,
    );
 ...
 * Generate summary report
 */
function generateReport(results: Record<string, Array<{ success?: boolean }>>) {
  console.log('\n' + '='.repeat(60));
  ...
  for (const [priority, priorityResults] of Object.entries(results)) {
    const success = priorityResults.filter((r) => r.success).length;
    const failed = priorityResults.filter((r) => !r.success).length;
  ...
  console.log(
    `   Overall Success Rate: ${((totalSuccess / (totalSuccess + totalFailed)) * 100).toFixed(1)}%`,
  );
...
export async function generateImagesByCategory(category: string) {
  ...
  return processPriorityGroup(requirements, coordinator, pipeline, `Category: ${category}`);
}
