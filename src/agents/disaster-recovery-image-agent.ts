...
export async function analyzePageForImages(pagePath: string): Promise<{
  ...
  const requiredImages =
    (PAGE_IMAGE_REQUIREMENTS as Record<string, Record<string, any>>)[pageType] || {};
 ...
 */
export function generateOptimal3DPrompt(context: {
  service?: string;
  location?: string;
  category?: string;
  imageType: string;
  ...
  if (service) {
    const servicePrompts: Record<string, string> = {
      'water-damage':
        'powerful water extraction equipment in action, industrial dehumidifiers with visible air flow, moisture meters showing readings, ',
      'fire-damage':
        'smoke damage restoration with HEPA filtration systems, soot removal equipment, thermal foggers creating visible cleaning mist, ',
      'mould-remediation':
        'HEPA air scrubbers with visible filtration, containment barriers with negative pressure, technicians in full PPE suits, ',
      'flood-recovery':
        'submersible pumps removing flood water, truck-mounted extraction units, infrared cameras detecting moisture, ',
      'storm-damage':
        'emergency tarping on damaged roof, tree removal equipment, structural stabilisation systems, ',
      'sewage-cleanup':
        'biohazard containment systems, industrial sanitization equipment, waste removal apparatus, ',
      'biohazard-cleaning':
        'specialised decontamination chambers, UV-C sterilisation lights, medical-grade cleaning systems, ',
      'trauma-scene':
        'discrete professional service vehicles, specialised cleaning equipment, privacy screens, ',
      'emergency-board-up':
        'rapid response vehicles, professional boarding materials, security systems being installed, ',
      'contents-restoration':
        'ultrasonic cleaning tanks, document drying chambers, electronics restoration stations, ',
    };

    prompt +=
      servicePrompts[service] || `${service.replace(/-/g, ' ')} professional service in progress, `;
  }
...
export function generateSEOMetadata(
  ...
  } - Professional 3D rendered ${imageType} image by Disaster Recovery`.trim();
  ...
  const schema = {
    ...
    creditText: 'Disaster Recovery - Professional 3D Visualisation',
    copyrightNotice: '© 2025 Disaster Recovery. All rights reserved.',
    keywords: [
      service?.replace(/-/g, ' '),
      location,
      'disaster recovery',
...
export class ImageGenerationOrchestrator {
  ...
  async analyzeAllPages(directory: string): Promise<{
    ...
    for (const page of pages) {
      ...
      allMissing.push(
        ...analysis.missingImages.map((img) => ({
          ...img,
          page: page,
        })),
      );
    ...
    // Sort by priority
    const priorityImages = allMissing.sort((a, b) => b.priority - a.priority).slice(0, 100); // Top 100 priority images
  ...
  private async saveProgress(): Promise<void> {
    ...
    await fs.writeFile('image-generation-progress.json', JSON.stringify(progress, null, 2));
  }
}
...
function detectPageType(
  pagePath: string,
  content: string,
): 'service' | 'location' | 'category' | 'other' {
...
function generateOptimalPrompt(pagePath: string, imageType: string, requirements: any): string {
  ...
  const service = pathParts.find(
    (p) => p.includes('-damage') || p.includes('-recovery') || p.includes('-cleanup'),
  );
