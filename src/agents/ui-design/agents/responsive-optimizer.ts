...
export class ResponsiveOptimizerAgent implements UIAgent {
  ...
  description =
    'Ensures mobile-first responsive design with optimal layouts across all device sizes';
  capabilities = [
  ...
  async execute(context: UIContext): Promise<AgentResult> {
    ...
    try {
      ...
      const mobileIssues = responsiveAnalysis.issues.filter(
        (issue) => issue.type === 'responsive' || issue.description.includes('mobile'),
      );

      if (mobileIssues.length > 0) {
        warnings.push(
          `Found ${mobileIssues.length} mobile usability issues that may impact user experience`,
        );
      }
      ...
      errors.push(
        `Responsive optimisation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      return {
        agentId: this.id,
        success: false,
  ...
  private analyzeMobileFirst(element: UIElement) {
    ...
    const hasMobileFirst =
      styleString.includes('@media (min-width') && !styleString.includes('@media (max-width');
    ...
    if (element.type === 'html' || element.type === 'document') {
      const hasViewportMeta = styleString.includes('viewport') || element.props?.viewport;
  ...
  private analyzeBreakpoints(element: UIElement) {
    ...
    const usesR6Breakpoints = r6Breakpoints.some(
      (bp) => styleString.includes(bp) || styleString.includes(bp.replace('px', 'em')),
    );
  ...
  private analyzeTouchTargets(element: UIElement) {
    ...
    const isInteractive =
      ['button', 'link', 'input', 'select'].includes(element.type) ||
  ...
  private analyzeResponsiveTypography(element: UIElement) {
    ...
    const hasFluidTypography =
      styleString.includes('clamp(') || styleString.includes('vw') || styleString.includes('vh');
