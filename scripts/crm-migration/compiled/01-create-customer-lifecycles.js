...
async function createCustomerLifecycles() {
    ...
    for (const user of users) {
        try {
            ...
            const hasCompletedBookings = user.bookings.some((b) => b.status === 'COMPLETED');
            ...
            const totalValue = user.bookings.reduce((sum, b) => sum + Number(b.finalCostAUD || b.estimatedCostAUD), 0);
            const completedJobs = user.bookings.filter((b) => b.status === 'COMPLETED').length;
            const averageJobValue = completedJobs > 0 ? totalValue / completedJobs : 0;
            const lastBooking = user.bookings.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0];
