...
const customer_lifecycle_service_1 = require("../../src/lib/crm/customer-lifecycle.service");
const prisma = new client_1.PrismaClient();
const customerLifecycleService = new customer_lifecycle_service_1.CustomerLifecycleService(prisma);
async function calculateHealthScores() {
    ...
    for (const lifecycle of lifecycles) {
        try {
            ...
            const healthScore = await customerLifecycleService.calculateHealthScore(lifecycle.id);
            console.log(`✅ Health score for ${lifecycle.userId}: ${healthScore}`);
            healthScoresCalculated++;
            // Calculate churn risk
            const churnRisk = await customerLifecycleService.calculateChurnRisk(lifecycle.id);
            console.log(`   Churn risk: ${churnRisk} ${churnRisk >= 50 ? '⚠️  AT RISK' : '✓'}`);
            churnRisksCalculated++;
        }
        catch (error) {
            console.error(`❌ Error processing lifecycle ${lifecycle.id}:`, error.message);
            errors++;
        }
    }
    console.log(`\n📊 Summary:`);
