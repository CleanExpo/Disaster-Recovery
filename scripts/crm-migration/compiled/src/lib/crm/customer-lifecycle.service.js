...
exports.customerLifecycleService = exports.CustomerLifecycleService = void 0;
...
class CustomerLifecycleService {
     ...
     */
    async getOrCreateLifecycle(userId) {
        ...
        try {
            ...
            if (!lifecycle) {
                advanced_logging_1.AdvancedLogger.info('Creating new customer lifecycle', { userId });
                ...
                lifecycle = await this.prisma.customerLifecycle.create({
                    data: {
                        ...
                        totalInteractions: 0,
                        lifetimeValueAUD: new client_1.Prisma.Decimal(0),
                        averageJobValueAUD: new client_1.Prisma.Decimal(0),
            ...
            advanced_logging_1.AdvancedLogger.error('Failed to get/create customer lifecycle', error, { userId });
            throw error;
        }
    }
    /**
     ...
     */
    async calculateHealthScore(lifecycleId) {
        ...
        if (lifecycle.lastInteractionDate) {
            const daysSince = Math.floor((Date.now() - lifecycle.lastInteractionDate.getTime()) / (1000 * 60 * 60 * 24));
            if (daysSince <= 7) {
                score += 30;
        ...
        const completedBookings = lifecycle.user.bookings.filter((b) => b.status === 'COMPLETED');
        const paidPayments = lifecycle.user.payments.filter((p) => p.status === 'COMPLETED');
        ...
        advanced_logging_1.AdvancedLogger.info('Health score calculated', {
            lifecycleId,
            score,
     ...
     */
    async calculateChurnRisk(lifecycleId) {
        const lifecycle = await this.prisma.customerLifecycle.findUnique({
            where: { id: lifecycleId },
            include: {
                activities: {
        ...
        const negativeSentimentCount = lifecycle.activities.filter((a) => a.sentiment === 'negative').length;
        ...
        advanced_logging_1.AdvancedLogger.info('Churn risk calculated', {
            lifecycleId,
            risk,
            isAtRisk,
            reasons,
     ...
     */
    async updateStage(lifecycleId, newStage) {
        const lifecycle = await this.prisma.customerLifecycle.findUnique({
            where: { id: lifecycleId },
        });
        if (!lifecycle) {
        ...
        if (!allowedTransitions.includes(newStage)) {
            throw new Error(`Invalid stage transition: ${lifecycle.currentStage} → ${newStage}`);
        }
        ...
        advanced_logging_1.AdvancedLogger.info('Lifecycle stage updated', {
            lifecycleId,
    ...
    async getCustomer360(userId) {
        ...
        const metrics = {
            ...
            openOpportunities: lifecycle.opportunities.filter((o) => !['CLOSED_WON', 'CLOSED_LOST'].includes(o.stage)).length,
     ...
     * @param lifecycleId - CustomerLifecycle ID
     */
    async recordInteraction(lifecycleId) {
        const updated = await this.prisma.customerLifecycle.update({
            where: { id: lifecycleId },
            data: {
     ...
     */
    async recordRevenue(lifecycleId, amountAUD) {
        const lifecycle = await this.prisma.customerLifecycle.findUnique({
            where: { id: lifecycleId },
        });
        if (!lifecycle) {
        ...
        const updated = await this.prisma.customerLifecycle.update({
            where: { id: lifecycleId },
            data: {
                lifetimeValueAUD: new client_1.Prisma.Decimal(newLifetimeValue),
                totalJobsCompleted: newJobCount,
                averageJobValueAUD: new client_1.Prisma.Decimal(newAverageValue),
            },
        });
        advanced_logging_1.AdvancedLogger.info('Revenue recorded', {
            lifecycleId,
            amountAUD,
            newLifetimeValue,
        });
    ...
    async runDailyHealthCheck() {
        const correlationId = advanced_logging_1.AdvancedLogger.setCorrelationId();
        advanced_logging_1.AdvancedLogger.info('Starting daily health check for all customers');
        ...
        for (const lifecycle of allLifecycles) {
            try {
                ...
                if (!lifecycle.isAtRisk && newChurnRisk >= 50) {
                    newAtRisk++;
                    advanced_logging_1.AdvancedLogger.warn('Customer became at-risk', {
                ...
                advanced_logging_1.AdvancedLogger.error('Failed to update customer health', error, {
                    lifecycleId: lifecycle.id,
                });
            }
        }
        advanced_logging_1.AdvancedLogger.info('Daily health check completed', {
    ...
    async getLifecycleMetrics() {
        const [totalCustomers, byStage, avgHealthScore, atRiskCount, advocateCount,] = await Promise.all([
...
exports.customerLifecycleService = new CustomerLifecycleService(new client_1.PrismaClient());
