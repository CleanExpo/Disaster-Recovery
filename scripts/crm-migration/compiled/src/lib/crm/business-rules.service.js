...
const advanced_logging_1 = require("../logger/advanced-logging");
const customer_lifecycle_service_1 = require("./customer-lifecycle.service");
class BusinessRulesService {
    constructor(prisma) {
        this.prisma = prisma;
        this.customerLifecycleService = new customer_lifecycle_service_1.CustomerLifecycleService(prisma);
    }
    /**
    ...
    async createRule(input) {
        ...
        advanced_logging_1.AdvancedLogger.info('Business rule created', {
            ruleId: rule.id,
            type: input.ruleType,
        });
        return rule;
     ...
     */
    async initializeDefaultRules(adminUserId) {
        const defaultRules = [
            {
        ...
        const created = await Promise.all(defaultRules.map((rule) => this.createRule({ ...rule, ownerId: adminUserId })));
        advanced_logging_1.AdvancedLogger.info('Default business rules initialized', {
    ...
    async evaluateAllRules() {
        const correlationId = advanced_logging_1.AdvancedLogger.setCorrelationId();
        advanced_logging_1.AdvancedLogger.info('Starting business rules evaluation');
        ...
        for (const rule of activeRules) {
            try {
                ...
                for (const violation of violations) {
                    const actionsExecuted = await this.executeViolationActions(rule, violation);
                    results.actionsExecuted += actionsExecuted;
                }
            }
            catch (error) {
                advanced_logging_1.AdvancedLogger.error('Rule evaluation failed', error, {
                    ruleId: rule.id,
                    ruleType: rule.ruleType,
                });
            }
        }
        advanced_logging_1.AdvancedLogger.info('Business rules evaluation completed', results);
        return results;
    }
    /**
    ...
    async evaluateRule(rule) {
        switch (rule.ruleType) {
            ...
            default:
                advanced_logging_1.AdvancedLogger.warn('Unknown rule type', { ruleType: rule.ruleType });
                return [];
        }
    }
    /**
    ...
    async checkResponseTime(rule) {
        ...
        for (const opp of staleOpportunities) {
            const minutesSinceCreation = Math.floor((Date.now() - opp.createdAt.getTime()) / (1000 * 60));
    ...
    async checkConversionRate(rule) {
        ...
        const wonCount = closedOpportunities.find((s) => s.stage === 'CLOSED_WON')?._count || 0;
        const lostCount = closedOpportunities.find((s) => s.stage === 'CLOSED_LOST')?._count || 0;
        const totalClosed = wonCount + lostCount;
        if (totalClosed === 0)
            return []; // No data yet
        const actualConversionRate = (wonCount / totalClosed) * 100;
        // Check if below threshold
        const isViolation = rule.comparison === 'greater_than'
        ...
        if (isViolation) {
            const violation = await this.createViolation({
                ...
                severity: actualConversionRate < targetPercent * 0.8 ? 'CRITICAL' : 'WARNING',
            });
            return [violation];
        }
        return [];
    ...
    async createViolation(data) {
        ...
        advanced_logging_1.AdvancedLogger.warn('Business rule violation detected', {
     ...
     */
    async executeViolationActions(rule, violation) {
            ...
            try {
                switch (action) {
                    ...
                    case 'create_report':
                        ...
                        break;
                    default:
                        advanced_logging_1.AdvancedLogger.warn('Unknown action type', { action });
                }
            }
            catch (error) {
                advanced_logging_1.AdvancedLogger.error('Action execution failed', error, { action });
            }
        }
     ...
     */
    async sendAlert(violation, rule) {
        advanced_logging_1.AdvancedLogger.warn('ALERT: Business rule violation', {
            ruleName: rule.name,
     ...
     */
    async createTask(violation, rule) {
        ...
        advanced_logging_1.AdvancedLogger.info('Task created for violation', {
            violationId: violation.id,
        });
    }
    /**
     ...
     */
    async notifyManager(violation, rule) {
        // TODO: Send email/SMS to rule owner
        advanced_logging_1.AdvancedLogger.warn('Manager notification sent', {
     ...
     */
    async createReport(violation, rule) {
        // TODO: Generate PDF report
        advanced_logging_1.AdvancedLogger.info('Report generated for violation', {
            violationId: violation.id,
        });
    }
    /**
     ...
     */
    async getActiveViolations(severity) {
     ...
     */
    async resolveViolation(violationId, resolvedById, resolutionNote) {
        ...
        advanced_logging_1.AdvancedLogger.info('Violation resolved', {
            violationId,
            resolvedById,
        });
        return updated;
...
exports.businessRulesService = new BusinessRulesService(new client_1.PrismaClient());
