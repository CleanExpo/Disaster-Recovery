...
import { PrismaClient, OpportunityStage, AustralianServiceType, EmergencyResponseLevel, AustralianState, ServiceStatus } from '@prisma/client';

const prisma = new PrismaClient();

function mapStatusToStage(status: ServiceStatus): OpportunityStage {
  const mapping: Record<ServiceStatus, OpportunityStage> = {
    PENDING: 'DISCOVERY',
    MATCHED: 'ASSESSMENT',
    ACCEPTED: 'PROPOSAL',
...
function extractState(location: string): AustralianState {
